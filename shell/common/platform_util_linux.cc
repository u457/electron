...
bool XDGUtil(const std::vector<std::string>& argv,
             ...
             base::Optional<platform_util::OpenCallback> callback) {
  ...
  if (wait_for_exit || callback.has_value()) {
    ...
    if (callback.has_value())
      std::move(callback.value()).Run(GetErrorDescription(exit_code));
...
bool XDGEmail(const std::string& email, const bool wait_for_exit) {
  return XDGUtil({"xdg-email", email}, wait_for_exit, base::nullopt);
}
